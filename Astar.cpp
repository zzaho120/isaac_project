...
void Astar::addOpenList()
{
	...
	{
		if (tile[Ci - 1][Cj].walkable)//상단 가운데 타일이 지나갈수 있다면
		{
			if (!tile[Ci - 1][Cj].listOn)//오픈리스트에 포함이 안되어 있는 타일이라면
			{
				tile[Ci - 1][Cj].listOn = true;//오픈리스트에 포함되었다.
				tile[Ci - 1][Cj].color = RGB(220, 255, 220);
				tile[Ci - 1][Cj].G = Cg + 10;//타일의G값을 클로즈 리스트의 누적 G+10
				tile[Ci - 1][Cj].parent = _closeList[_lastIndex];//타일의 부모를 클로즈 리스트의 마지막으로 추가
				_openList.push_back(&tile[Ci - 1][Cj]);//오픈 리스트에 추가
			}
			else//오픈리스트에 포함이 되어 있던 타일이라면
			{
				if (Cg + 10 < tile[Ci - 1][Cj].G)//기존G값보다 새로 계산한 G값이 작다면
				{
					tile[Ci - 1][Cj].G = Cg + 10;//G값 새롭게 계산
		...
		if (Cj!=0)//좌상단 : 0번째 열이 아니라면
		{
			...
			if (tile[Ci - 1][Cj - 1].walkable && tile[Ci][Cj].walkable && tile[Ci][Cj - 1].walkable)
			{
				if (!tile[Ci - 1][Cj - 1].listOn)
				{
					...
					tile[Ci - 1][Cj - 1].G = Cg + 14;//대각선 이므로
	...
	{
		if (tile[Ci][Cj + 1].walkable)//우측타일이 이동가능하다면
		{
			if (!tile[Ci][Cj + 1].listOn)
			{
				tile[Ci][Cj + 1].listOn = true;
